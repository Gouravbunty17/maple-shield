...
- [ ] Have a video file ready (drone footage) OR use webcam
...
> Or for webcam:
```
python maple_shield_mvp.py
```

**Point out on screen:**
- Bounding boxes appearing around detected objects
- Track IDs staying consistent across frames (e.g. "ID 1", "ID 2")
- Threat level label: **CLEAR → CAUTION → WARNING → CRITICAL**
- FPS counter showing real-time performance
- JSONL log file being written

**Say:**
> "The system detects objects in real time using a YOLOv8 model we trained
> specifically for drone detection — achieving 92.6% accuracy. Each detection
...
> command and control systems including STANAG 4586. This is the feed
> a base operator would see on their screen."
...
- [ ] Add the link to your IDEaS application
...
| Zoom in on key numbers (92.6%, threat levels) | Leave irrelevant windows visible |

...
def postprocess_yolov8(out, conf_thresh=0.35, iou_thresh=0.45, max_det=50):
    # out expected shape: (1, 84, 8400)
    pred = out[0].transpose(1, 0).astype(np.float32)  # (8400, 84)
    boxes_xywh = pred[:, 0:4]
    cls_scores = pred[:, 4:]
...
def main():
    ...
    print("📊 Risk scoring enabled (confirmed tracks only)")
    ...
    while True:
        ...
        # -----------------------------
        # Compute risk scores (CONFIRMED TRACKS ONLY)
        # -----------------------------
        for d in det_list:
            if not d.get("track_confirmed", False):
                d["risk"] = {"risk_score": 0.0, "risk_state": "SAFE"}
                continue
            d["risk"] = compute_risk_score(d["box"], d["conf"], w, h)

        # -----------------------------
        # Determine MAX RISK (robust)
        # -----------------------------
        max_risk = 0.0
        max_risk_state = "SAFE"
        if det_list:
            best = max(det_list, key=lambda x: x.get("risk", {}).get("risk_score", 0.0))
            max_risk = float(best.get("risk", {}).get("risk_score", 0.0))
            max_risk_state = str(best.get("risk", {}).get("risk_state", "SAFE"))
        ...
        for d in det_list:
            ...
            label = f"ID{tid} {lbl} {conf:.2f} | {risk_state} {risk_score:.2f}"
        ...
        # -----------------------------
        header = (
            f"FPS {fps:.1f} | {infer_ms:.1f}ms | "
            f"{len(det_list)} det | MAX RISK: {max_risk_state} {max_risk:.2f}"
        )
